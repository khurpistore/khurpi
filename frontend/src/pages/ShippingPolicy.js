...
const ShippingPolicy = () => {
  ...
  const fetchContent = async () => {
    try {
      const response = await axios.get(`${API}/pages/shipping-policy`);
          ...
          <CardContent className="p-6 sm:p-8">
            <div className="prose prose-green max-w-none">
              {content?.content ? (
                ...
                <p className="text-muted-foreground">No shipping policy content available.</p>
              )}
            </div>
          </CardContent>
        </Card>
