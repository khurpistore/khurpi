...
import { CalendarIcon, Check, Package, ChevronLeft, ChevronRight, Sparkles, Tag, Truck, CreditCard, Gift, X, Loader2, MapPin, Plus } from 'lucide-react';
        ...
        {step === 2 && (
          <div>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-3 mb-4">
                  {selectedProducts.map((item) => {
                    const product = products.find(p => p.id === item.product_id);
                    return product ? (
                      <div key={item.product_id} className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100">
                        <img
                          src={product.image}
                          alt={product.name}
                ...
                {/* Price Calculation - only show when plan is selected */}
                {selectedPlan && (
                  <>
                    <div className="bg-white rounded-lg p-3 border border-gray-100 mb-4">
                      <p className="text-xs text-muted-foreground mb-2 uppercase tracking-wide">Monthly Price Calculation</p>
                      <div className="flex items-center justify-center gap-2 text-sm flex-wrap">
                        <div className="text-center px-3 py-2 bg-gray-50 rounded-lg">
                          <p className="font-bold text-primary">₹{calculatePerTrayPrice().toFixed(0)}</p>
                          <p className="text-xs text-muted-foreground">per tray</p>
                        </div>
                        <span className="text-muted-foreground font-bold">×</span>
                        <div className="text-center px-3 py-2 bg-gray-50 rounded-lg">
                          <p className="font-bold text-primary">{selectedPlan.deliveries_per_week}</p>
                        ...
                        <div className="text-center px-3 py-2 bg-primary/10 rounded-lg">
                          <p className="font-bold text-primary">₹{calculateMonthlySubtotal().toFixed(0)}</p>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between py-1">
                        ...
                        <span className="font-medium">₹{calculateMonthlySubtotal().toFixed(2)}</span>
                      </div>
                      ...
                      {selectedPlan.discount > 0 && (
                        <div className="flex justify-between py-1 text-green-600 bg-green-50 px-2 -mx-2 rounded">
                          <span className="flex items-center gap-1">
                            ...
                            Plan Discount ({selectedPlan.discount}%)
                          </span>
                      ...
                      <div className="flex justify-between py-1">
                        ...
                        {getDeliveryFee() === 0 ? (
                          ...
                          <span className="font-medium">₹{(getDeliveryFee() * 4).toFixed(2)}</span>
                        )}
                      </div>
                      ...
                      <div className="flex justify-between items-center pt-3 mt-2 border-t-2 border-dashed border-primary/30">
                        <div>
                          ...
                          {selectedPlan.discount > 0 && (
                            <p className="text-xs text-green-600">You save ₹{calculateDiscount().toFixed(0)}/month!</p>
                          )}
                        </div>
                        <span className="font-bold text-2xl text-primary">₹{calculateTotal().toFixed(0)}<span className="text-sm font-normal text-muted-foreground">/mo</span></span>
                      </div>
                    </div>
                  </>
                )}

                {/* Show simple per-delivery cost when no plan selected */}
                {!selectedPlan && (
            ...
            {selectedPlan && selectedProducts.length > 0 && (
              <Card className="mt-6 border-2 border-primary/20 bg-gradient-to-br from-green-50/50 to-white overflow-hidden">
                <div className="bg-primary/10 px-4 py-3 border-b border-primary/20">
                  <h4 className="font-semibold text-primary flex items-center gap-2">
