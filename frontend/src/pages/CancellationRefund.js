...
const CancellationRefund = () => {
  ...
  const fetchContent = async () => {
    try {
      const response = await axios.get(`${API}/pages/cancellation-refund`);
      setContent(response.data);
    } catch (error) {
      console.error('Failed to fetch cancellation and refund policy');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-primary">
            {content?.title || 'Cancellations and Refunds'}
          </h1>
        </div>
              ...
              {content?.content ? (
                <ReactMarkdown
                  components={{
                    ...
                    th: ({children}) => <th className="border border-gray-300 px-4 py-2 bg-gray-100 text-left">{children}</th>,
                ...
                <p className="text-muted-foreground">No cancellation and refund policy content available.</p>
              )}
            </div>
          </CardContent>
        </Card>
